...
export default function DeliveryDispatchBoard() {
  ...
  async function loadQueue() {
    ...
    if (!res.ok) throw new Error(data?.error || "Failed to load delivery queue");

    const rows = Array.isArray(data?.data) ? data.data : [];
    setQueue(
      rows.filter((row: any) =>
        ["SUBMITTED", "SAVED", "IN_TRANSIT"].includes(String(row.delivery_status || row.status || "").toUpperCase()) &&
        !row.run_sheet_id
      )
    );
  }
  ...
  async function loadSheets() {
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Failed to load run sheets");
  ...
  async function refreshAll() {
    ...
    try {
      ...
      setMessage(error?.message || "Failed to refresh dispatch board");
    }
  }
  ...
  async function submit(action: "create_run_sheet" | "dispatch_run_sheet") {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Failed to save run sheet");
      ...
      setMessage(error?.message || "Failed to submit run sheet");
    }
  }
      ...
      <section style={{ ...card, display: "flex", justifyContent: "space-between", gap: 18, alignItems: "flex-start" }}>
        <div>
          ...
          <p style={{ margin: "10px 0 0", color: "#64748b", fontSize: 14, lineHeight: 1.7 }}>
            {tr("Build one Run Sheet for many delivery IDs, assign rider details, and dispatch them to Out for Delivery in batch.")}
          </p>
        </div>
      </section>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(380px, 1fr) minmax(0, 1.1fr)", gap: 18 }}>
        <section style={card}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 16 }}>
            <div style={{ fontSize: 22, fontWeight: 900, color: "#0f172a" }}>{tr("Dispatch Queue")}</div>
            <input
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ marginTop: 6, color: "#64748b", fontSize: 12 }}>
                      {safe(row.pickup_id)} · {safe(row.receiver_township || row.township)}
                    </div>
                  </div>
                </label>
              );
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <section style={card}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
              <input style={inputStyle} type="date" value={form.route_date} onChange={(e) => setForm({ ...form, route_date: e.target.value })} />
              <input style={inputStyle} placeholder={tr("Hub")} value={form.hub_code} onChange={(e) => setForm({ ...form, hub_code: e.target.value })} />
              <input style={inputStyle} placeholder={tr("Rider Name")} value={form.rider_name} onChange={(e) => setForm({ ...form, rider_name: e.target.value })} />
              <input style={inputStyle} placeholder={tr("Rider Phone")} value={form.rider_phone} onChange={(e) => setForm({ ...form, rider_phone: e.target.value })} />
              <input style={inputStyle} placeholder={tr("Vehicle No")} value={form.vehicle_no} onChange={(e) => setForm({ ...form, vehicle_no: e.target.value })} />
              <input style={inputStyle} placeholder={tr("Zone / Route")} value={form.zone_code} onChange={(e) => setForm({ ...form, zone_code: e.target.value })} />
            </div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 16 }}>
              <button style={primaryBtn} onClick={() => submit("create_run_sheet")}>
                {tr("Create Run Sheet")}
              </button>
              <button style={secondaryBtn} onClick={() => submit("dispatch_run_sheet")}>
          ...
          <section style={card}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 16 }}>
              <div style={{ fontSize: 22, fontWeight: 900, color: "#0f172a" }}>{tr("Existing Run Sheets")}</div>
              <div style={{ fontSize: 12, fontWeight: 800, color: "#475569" }}>{sheets.length} sheet(s)</div>
            </div>

            <div style={{ display: "flex", flexDirection: "column", gap: 12, maxHeight: 420, overflow: "auto" }}>
              {sheets.map((sheet: any) => (
                <div key={sheet.run_sheet_id} style={{ border: "1px solid #dbe4ee", borderRadius: 18, background: "#f8fafc", padding: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <strong style={{ color: "#0f172a" }}>{safe(sheet.run_sheet_id)}</strong>
                    <span style={{ fontSize: 11, fontWeight: 800, color: "#475569", textTransform: "uppercase" }}>
                      {statusText(lang, sheet.status)}
                    </span>
                  </div>
                  <div style={{ marginTop: 6, color: "#334155" }}>
                    {safe(sheet.route_date)} · {safe(sheet.hub_code)} · {tr("Rider")}: {safe(sheet.rider_name)}
                  </div>
                  <div style={{ marginTop: 6, color: "#64748b", fontSize: 12 }}>
                    Items: {Array.isArray(sheet.delivery_run_sheet_items) ? sheet.delivery_run_sheet_items.length : 0}
                  </div>
                </div>
              ))}
              {!sheets.length && (
