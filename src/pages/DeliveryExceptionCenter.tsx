...
export default function DeliveryExceptionCenter() {
  ...
  const [reattemptAt, setReattemptAt] = useState(new Date().toISOString().slice(0, 16));
  ...
  async function loadRows() {
    ...
    if (!res.ok) throw new Error(data?.error || "Failed to load exception queue");
  ...
  async function runAction(action: "schedule_reattempt" | "verify_pod" | "reject_pod" | "mark_rts") {
    ...
    try {
      const body: any = {
        ...
        reattempt_scheduled_at: reattemptAt ? new Date(reattemptAt).toISOString() : "",
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Failed to update exception workflow");

      setSelected(data.data);
      setMessage(`Action completed: ${action} for ${data.data.delivery_id}`);
      await loadRows();
    } catch (error: any) {
      setMessage(error?.message || "Failed to run exception action");
    }
  }
      ...
      <section style={{ ...card, display: "flex", justifyContent: "space-between", gap: 18, alignItems: "flex-start" }}>
        <div>
          ...
          <h1 style={{ margin: "14px 0 0", fontSize: 30, fontWeight: 900, color: "#0f172a" }}>
            Failed Attempts, POD Review, and Return to Sender
          </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(360px,.95fr) minmax(0,1.15fr)", gap: 18 }}>
        <section style={card}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 16 }}>
            <div style={{ fontSize: 22, fontWeight: 900, color: "#0f172a" }}>Exception Queue</div>
        ...
        <section style={{ ...card, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: "#0f172a" }}>Exception Action Panel</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 12 }}>
            ...
            <Metric label="Status" value={statusText(lang, selected?.delivery_status)} />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <input style={inputStyle} type="datetime-local" value={reattemptAt} onChange={(e) => setReattemptAt(e.target.value)} />
            <input style={inputStyle} placeholder={tr("Reviewed by")} value={reviewedBy} onChange={(e) => setReviewedBy(e.target.value)} />
          </div>

          <textarea style={{ ...inputStyle, minHeight: 90 }} placeholder={tr("Reattempt note / review note")} value={reattemptNote} onChange={(e) => setReattemptNote(e.target.value)} />
          <textarea style={{ ...inputStyle, minHeight: 90 }} placeholder={tr("POD review note")} value={reviewNote} onChange={(e) => setReviewNote(e.target.value)} />
          <textarea style={{ ...inputStyle, minHeight: 90 }} placeholder={tr("RTS reason")} value={rtsReason} onChange={(e) => setRtsReason(e.target.value)} />

          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button style={primaryBtn} onClick={() => runAction("schedule_reattempt")}>{tr("Schedule Reattempt")}</button>
            <button style={primaryBtn} onClick={() => runAction("verify_pod")}>{tr("Verify POD")}</button>
            <button style={secondaryBtn} onClick={() => runAction("reject_pod")}>{tr("Reject POD")}</button>
            <button style={secondaryBtn} onClick={() => runAction("mark_rts")}>{tr("Mark RTS")}</button>
          </div>
        </section>
      </div>
    </div>
