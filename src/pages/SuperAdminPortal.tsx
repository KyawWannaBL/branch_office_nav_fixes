...
export default function SuperAdminPortal() {
  ...
  const goLiveChecklist = useMemo(() => {
    ...
    return [
      { label: "Operations Command API", ok: !!api.commandCenter },
      ...
      { label: "Tariff Master API", ok: !!api.tariffs },
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <section
        style={{
          ...
          background: "linear-gradient(135deg,#ffffff 0%,#f8fbff 50%,#fefce8 100%)",
        }}
      >
        <div>
          <div
          ...
          >
            {tr("Super Administration")}
          </div>
          <h1 style={{ margin: "14px 0 0", fontSize: 32, fontWeight: 900, color: "#0f172a" }}>
            {tr("Super Admin Portal")}
          </h1>
          <p style={{ margin: "10px 0 0", color: "#64748b", fontSize: 14, lineHeight: 1.7 }}>
            {tr("Final go-live control tower for system readiness, privileged oversight, operational health, master data confidence, and critical audit supervision.")}
          </p>
        </div>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14 }}>
        <KpiCard icon={<Crown size={18} />} label={tr("Open Ways")} value={String(stats.openWays)} tone="info" />
        ...
        <KpiCard icon={<Package size={18} />} label={tr("Delivered")} value={String(stats.delivered)} tone="good" />
        <KpiCard icon={<Database size={18} />} label={tr("COD Open")} value={`${money(stats.codOpen)} MMK`} tone="info" />
        <KpiCard icon={<FileCheck2 size={18} />} label={tr("Tariff Rows")} value={String(stats.tariffCount)} tone="good" />
        <KpiCard icon={<ShieldCheck size={18} />} label={tr("Audit Events Today")} value={String(stats.auditToday)} />
      </section>

      <div style={{ display: "grid", gridTemplateColumns: "1.05fr .95fr", gap: 18 }}>
        <section style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <Panel title={tr("Operations Command Snapshot")}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
              ...
              <DetailMetric label={tr("Tariff Rows")} value={String(stats.tariffCount)} />
            </div>
          </Panel>

          <Panel title={tr("Recent Dispatch Control")}>
            {recentBatches.length ? (
              ...
              <div style={{ textAlign: "center", color: "#64748b", padding: 18 }}>
                {loading ? tr("Loading super admin portal...") : tr("No dispatch batches found.")}
              </div>
            )}
          </Panel>
          ...
          <Panel title={tr("Rider Load Supervision")}>
            {riderSummary.length ? (
              ...
              <div style={{ textAlign: "center", color: "#64748b", padding: 18 }}>
                {loading ? tr("Loading super admin portal...") : tr("No rider summary found.")}
              </div>
            )}
          </Panel>
        </section>
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <Panel title={tr("Exception Watchlist")}>
            {recentExceptions.length ? (
              ...
              <div style={{ textAlign: "center", color: "#64748b", padding: 18 }}>
                {loading ? tr("Loading super admin portal...") : tr("No exception rows found.")}
              </div>
            )}
          </Panel>

          <Panel title={tr("Privileged Audit Feed")}>
            {adminAuditFeed.length ? (
              ...
              <div style={{ textAlign: "center", color: "#64748b", padding: 18 }}>
                {loading ? tr("Loading super admin portal...") : tr("No privileged audit feed available.")}
              </div>
            )}
          </Panel>
          ...
          <Panel title={tr("Go-Live Quick Actions")}>
            ...
            <ActionLink to="/admin-hr" label={tr("Open HR Portal")} />
            <ActionLink to="/marketing" label={tr("Open Marketing Portal")} />
            <ActionLink to="/merchant" label={tr("Open Merchant Portal")} />
          </Panel>
        </section>
      </div>
    </div>
